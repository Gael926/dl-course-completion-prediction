...
print(
    f"Données Classification: Train={X_train_class_tensor.shape[0]}, Val={X_val_class_tensor.shape[0]}, Test={X_test_class_tensor.shape[0]}"
)
...
for epoch in range(500):
    ...
    if (epoch + 1) % 20 == 0:
        print(
            f"Epoch {epoch + 1:03d} | Train Loss: {loss.item():.4f} | Val Loss: {val_loss.item():.4f}"
        )
...
print(
    f"Données Classification: Train={X_train_reg.shape[0]}, Val={X_val_reg.shape[0]}, Test={X_test_reg.shape[0]}"
)
...
for epoch in range(500):
    ...
    if (epoch + 1) % 20 == 0:
        print(
            f"Epoch {epoch + 1:03d} | Train HuberLoss: {loss.item():.4f} | Val HuberLoss: {val_loss.item():.4f}"
        )
